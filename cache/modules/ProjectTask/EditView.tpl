...
<td valign="top" id='date_finish_label' width='12.5%' scope="col">
...
<td valign="top" width='37.5%' >
{counter name="panelFieldCount"}
<select accesskey=""  tabindex="0"  name="{$fields.status.name}" id="{$fields.status.name}" title="" tabindex="s" onchange="update_percent_complete(this.value);">{if isset($fields.status.value) && $fields.status.value != ""}{html_options options=$fields.status.options selected=$fields.status.value}{else}{html_options options=$fields.status.options selected=$fields.status.default}{/if}</select>
...
{capture name="tr" assign="tableRow"}
<tr>
<td valign="top" id='percent_complete_label' width='12.5%' scope="col">
...
<td valign="top" width='37.5%' colspan='3'>
{counter name="panelFieldCount"}
<input accesskey=""  tabindex="0"  type="text" name="{$fields.percent_complete.name}" id="{$fields.percent_complete.name}" size="30" value="{$fields.percent_complete.value}" title="" tabindex="0" onChange="update_status(this.value);" /></tr>
</tr>
{/capture}
{if $fieldsUsed > 0 }
{$tableRow}
...
{capture name="tr" assign="tableRow"}
<tr>
<td valign="top" id='milestone_flag_label' width='12.5%' scope="col">
...
{capture name="tr" assign="tableRow"}
<tr>
<td valign="top" id='project_name_label' width='12.5%' scope="col">
...
<span class="id-ff multiple">
<button type="button" name="btn_{$fields.project_name.name}" id="btn_{$fields.project_name.name}" tabindex="0" title="{sugar_translate label="LBL_SELECT_BUTTON_TITLE"}" class="button firstChild" value="{sugar_translate label="LBL_SELECT_BUTTON_LABEL"}"
onclick='open_popup(
...
"single", 
true
);' ><img src="{sugar_getimagepath file="id-ff-select.png"}"></button><button type="button" name="btn_clr_{$fields.project_name.name}" id="btn_clr_{$fields.project_name.name}" tabindex="0" title="{sugar_translate label="LBL_ACCESSKEY_CLEAR_RELATE_TITLE"}"  class="button lastChild"
...
<td valign="top" id='anticipos_label' width='12.5%' scope="col">
{capture name="label" assign="label"}{sugar_translate label='LBL_ANTICIPOS' module='ProjectTask'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
...
{if strlen($fields.anticipos.value) <= 0}
{assign var="value" value=$fields.anticipos.default_value }
{else}
{assign var="value" value=$fields.anticipos.value }
{/if}  
<input type='text' name='{$fields.anticipos.name}'
id='{$fields.anticipos.name}'
size='30'
...
{capture name="label" assign="label"}{sugar_translate label='LBL_ESCALAFON' module='ProjectTask'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.escalafon.name}" 
id="{$fields.escalafon.name}" 
title=''       
>
{if isset($fields.escalafon.value) && $fields.escalafon.value != ''}
{html_options options=$fields.escalafon.options selected=$fields.escalafon.value}
{else}
{html_options options=$fields.escalafon.options selected=$fields.escalafon.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.escalafon.options }
{capture name="field_val"}{$fields.escalafon.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.escalafon.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.escalafon.name}" 
id="{$fields.escalafon.name}" 
title=''          
>
{if isset($fields.escalafon.value) && $fields.escalafon.value != ''}
...
<input
id="{$fields.escalafon.name}-input"
name="{$fields.escalafon.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.escalafon.name}-image"></button><button type="button"
id="btn-clear-{$fields.escalafon.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.escalafon.name}-input', '{$fields.escalafon.name}');sync_{$fields.escalafon.name}()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
</span>
{literal}
<script>
SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
...
{literal}
(function (){
var selectElem = document.getElementById("{/literal}{$fields.escalafon.name}{literal}");
if (typeof select_defaults =="undefined")
select_defaults = [];
select_defaults[selectElem.id] = {key:selectElem.value,text:''};
//get default
...
YUI().use("autocomplete", "autocomplete-filters", "autocomplete-highlighters", "node","node-event-simulate", function (Y) {
{/literal}
SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.escalafon.name}-input');
SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.escalafon.name}-image');
SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.escalafon.name}');
{literal}
function SyncToHidden(selectme){
var selectElem = document.getElementById("{/literal}{$fields.escalafon.name}{literal}");
var doSimulateChange = false;
...
//global variable 
sync_{/literal}{$fields.escalafon.name}{literal} = function(){
SyncToHidden();
}
function syncFromHiddenToWidget(){
var selectElem = document.getElementById("{/literal}{$fields.escalafon.name}{literal}");
...
SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.simulate('keyup');
for (i=0;i<selectElem.options.length;i++){
if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.escalafon.name}-input{literal}'))
SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
}
}
YAHOO.util.Event.onAvailable("{/literal}{$fields.escalafon.name}{literal}", syncFromHiddenToWidget);
{/literal}
SUGAR.AutoComplete.{$ac_key}.minQLen = 0;
SUGAR.AutoComplete.{$ac_key}.queryDelay = 0;
SUGAR.AutoComplete.{$ac_key}.numOptions = {$field_options|@count};
...
{/literal}
{/if}
<td valign="top" id='utilization_label' width='12.5%' scope="col">
...
{capture name="tr" assign="tableRow"}
<tr>
<td valign="top" id='estimated_effort_label' width='12.5%' scope="col">
...
id='{$fields.estimated_effort.name}'
size='30'
value='{sugar_number_format var=$value  }'
title=''
tabindex='0'
...
<td valign="top" width='37.5%' colspan='3'>
{counter name="panelFieldCount"}
<input accesskey=""  tabindex="0"  type="hidden" name="duration" id="projectTask_duration" value="0" />
</tr>
{/capture}
{if $fieldsUsed > 0 }
{$tableRow}
...
<td valign="top" width='37.5%' colspan='3'>
{counter name="panelFieldCount"}
<input accesskey=""  tabindex="0"  type="hidden" name="duration_unit" id="projectTask_durationUnit" value="Days" />
</tr>
{/capture}
{if $fieldsUsed > 0 }
{$tableRow}
...
addToValidate('EditView', 'estimated_effort', 'float', false,'{/literal}{sugar_translate label='LBL_ESTIMATED_EFFORT' module='ProjectTask' for_js=true}{literal}' );
addToValidate('EditView', 'actual_effort', 'float', false,'{/literal}{sugar_translate label='LBL_ACTUAL_EFFORT' module='ProjectTask' for_js=true}{literal}' );
...
addToValidate('EditView', 'escalafon', 'enum', false,'{/literal}{sugar_translate label='LBL_ESCALAFON' module='ProjectTask' for_js=true}{literal}' );
...
addToValidate('EditView', 'anticipos', 'float', false,'{/literal}{sugar_translate label='LBL_ANTICIPOS' module='ProjectTask' for_js=true}{literal}' );
...
</script><script language="javascript">if(typeof sqs_objects == 'undefined'){var sqs_objects = new Array;}sqs_objects['EditView_assigned_user_name']={"form":"EditView","method":"get_user_array","field_list":["user_name","id"],"populate_list":["assigned_user_name","assigned_user_id"],"required_list":["assigned_user_id"],"conditions":[{"name":"user_name","op":"like_custom","end":"%","value":""}],"limit":"30","no_match_text":"Sin coincidencias"};sqs_objects['EditView_project_name']={"form":"EditView","method":"query","modules":["Project"],"group":"or","field_list":["name","id"],"populate_list":["project_name","project_id"],"required_list":["parent_id"],"conditions":[{"name":"name","op":"like_custom","end":"%","value":""}],"order":"name","limit":"30","no_match_text":"Sin coincidencias"};</script>{/literal}
