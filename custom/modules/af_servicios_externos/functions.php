...
class functions {
    	...
    	function subtotal(&$bean, $event, $arguments){
			$bean->subtotal = $bean->valor * $bean->cantidad * $bean->porcentaje;
	}
	function valor_cot(&$bean, $event, $arguments){
        	global $db;
		
		$id = $bean->id;
		$sql = "SELECT opportunities_af_servicios_externos_1opportunities_ida  AS opp FROM opportunities_af_servicios_externos_1_c WHERE opportunities_af_servicios_externos_1af_servicios_externos_idb = '$id' AND deleted = 0";
		$res = $db->query($sql);
		...
		$sql = "SELECT subtotal FROM af_servicios_externos WHERE id = '$id'";
		$res = $db->query($sql);
