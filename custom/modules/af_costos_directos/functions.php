...
class functions {
	...
	function del_valor_cot(&$bean, $event, $arguments){
		global $db;
		...
		$sql = "SELECT subtotal FROM af_costos_directos WHERE id = '$id' AND deleted=0";
		$res = $db->query($sql);
		$data = $db->fetchByAssoc($res);
	...
	function valor_cot(&$bean, $event, $arguments){
		...
		$sql = "SELECT subtotal FROM af_costos_directos WHERE id = '$id' deleted = 0";
		$res = $db->query($sql);
		$data = $db->fetchByAssoc($res);
		$total_costo_directo = $data['subtotal'];
